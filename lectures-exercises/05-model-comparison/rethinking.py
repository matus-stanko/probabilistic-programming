...
def standardize(series):
    ...
    std_series = (series - series.mean()) / series.std(ddof=1)
    return std_series

def unstandardize(series, original):
    """Move a standardized series to the original units. 
       series - a series to standardize
       oroginal - the original old series used to introduce standardized units
    """
    return series * original.std(ddof=1) + original.mean()
